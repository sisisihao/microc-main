...
let rec cStmt stmt (varEnv: VarEnv) (funEnv: FunEnv) (endStack: EndStack ): instr list =
    ...
    | If (e, stmt1, stmt2) ->
        ...
        @ [ IFZERO labelse ]
          @ cStmt stmt1 varEnv funEnv endStack
            @ [ GOTO labend ]
              @ [ Label labelse ]
                @ cStmt stmt2 varEnv funEnv endStack @ [ Label labend ]
    | For(e1, e2, e3, body) ->         
      let labbegin = newLabel()
      let labtest  = newLabel()
      let labend = newLabel()
      let endStack = labend :: endStack
      cExpr e1 varEnv funEnv @ [INCSP -1]
      @ [GOTO labtest; Label labbegin] @ cStmt body varEnv funEnv endStack
      @ cExpr e3 varEnv funEnv @ [INCSP -1]
      @ [Label labtest] @ cExpr e2 varEnv funEnv @ [IFNZRO labbegin]@[Label labend]
    | Break ->
    ...
    | While (e, body) ->
        ...
        let labtest = newLabel ()
        let labend = newLabel()
        let endStack1 = labend :: endStack
        
        [ GOTO labtest; Label labbegin ]
        @ cStmt body varEnv funEnv endStack1
          @ [ Label labtest ]
    ...
    | Block stmts ->
        ...
        let rec loop stmts varEnv =
            ...
            | [] -> (snd varEnv, [])
            | s1 :: sr ->
                let (varEnv1, code1) = cStmtOrDec s1 varEnv funEnv endStack
...
and cStmtOrDec stmtOrDec (varEnv: VarEnv) (funEnv: FunEnv) (endStack: EndStack): VarEnv * instr list =
    ...
    | Stmt stmt -> (varEnv, cStmt stmt varEnv funEnv endStack)
...
let cProgram (Prog topdecs) : instr list =
    ...
    let compilefun (tyOpt, f, xs, body) =
        ...
        let endStack = []
        let code = cStmt body (envf, fdepthf) funEnv endStack
