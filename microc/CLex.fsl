...
rule Token = parse          // 每个 规则rule 会生成为一个函数，函数名是规则名 Token
  ...
  | "__"['a'-'z''A'-'Z''0'-'9']*   {failwith "cannot use variable with start __ "}
  | ['a'-'z''A'-'Z']['a'-'z''A'-'Z''0'-'9']*
  ...
  | "(*"            { Comment_par lexbuf; Token lexbuf}
...
and Comment_par = parse
   | "(*"           { Comment_par lexbuf; Comment_par lexbuf }  // 注释的嵌套处理
   | "*)"           { () }                           // 注释处理结束
   | '\n'           { lexbuf.EndPos <- lexbuf.EndPos.NextLine; Comment_par lexbuf } //注释跨行处理
   | (eof | '\026') { failwith "Lexer error: unterminated comment" }  // 多行注释未封闭
   | _              { Comment_par lexbuf }  // 其他任意情况都继续处理后续字符
