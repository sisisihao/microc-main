...
and stmtordec stmtordec locEnv gloEnv store =
    ...
    | DecWithAssign(typ,x,e) -> 
        ...
        let (locIndex2 , store2) = access (AccVar x) locEnv1 gloEnv store
        let (res, store3) = eval e locEnv gloEnv store2
        (locEnv1, setSto store3 locIndex2 res)
...
let initEnvAndStore (topdecs: topdec list) : locEnv * funEnv * store =
    ...
    let rec addv decs locEnv funEnv store =
        ...
        | VarDecWithAssign (typ, x, e) :: decr ->
            //printf "\niter golbal env:\n"
            //  (locEnv, funEnv, store)
            //failwith(" fail ")
            let ((varEnv,nextloc), funEnv, store) = (locEnv,funEnv,store)
            let gloEnv1 = (fst(locEnv),funEnv)
            
            let (locEnv2, store1) = allocate (typ, x) locEnv store
            
            let (locIndex , store2) = access (AccVar x) locEnv2 gloEnv1 store1
        
            let (res, store3) = eval e locEnv2 gloEnv1 store2
        
            let store4 =  setSto store3 locIndex res
            //(topEnv,store4) = bindVar x res (varEnv,nextloc) store3
            //let (locEnv1, sto1) = allocate (typ, x) locEnv store
           // let (locEnv5, store5) = allocate (typ, x) locEnv2 store1
            addv decr locEnv2 funEnv store4
            //addv decr locEnv2 funEnv store4
